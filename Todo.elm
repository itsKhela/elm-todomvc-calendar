...
{-
  TODO:
    - fix date type
    - fix date movement
    - implement calendar html structure
-}
...
-- TODO: replace this with actual valid date from elm --
type alias Date =
    { day : Int
    , month : Int
    , year : Int
...
type alias Task =
    ...
    , completed : Bool
    , editing : Bool
-- not sure if date should go here also --
    , date : Date
    , id : Int
    }


newTask : String -> Int -> Date -> Task
newTask desc id date =
    { description = desc
    , completed = False
    , editing = False
    , date = date
...
-- TODO: Make sure "today" gets actual date value --
emptyModel : Date -> Model
emptyModel today =
    { tasks = []
    , visibility = "All"
    , field = ""
...
update action model =
    case action of
      ...
      NextYear date ->
          { model | date = date.year+1 }

      PrevYear date ->
          { model | date = date.year-1 }

      NextMonth date ->
          { model | date = date.month+1 }

      PrevMonth date ->
          { model | date = date.month-1 }

      NextDay date ->
          { model | date = date.day+1 }
...
-- Need to add here coherent lazy structure with the proper structure --          
          , lazy2 calendar address model.date
...
-- calendar must implement the proper functions to move the date --
calendar: Address Action -> Date -> Html
calendar address date = 
    div
      [ id "calendar" ]
      [ h1 [] [ text "calendar" ]
      , div []
          ...
          ,   [ input 
           ...
           ,   [ input 
                  ...
                  , type' "button"
                  , name "Previous Month"
                  , onClick address (PrevMonth date)
                  ]
                  []
              ]
           ,   [ input 
           ...
           ,   [ input 
                  ...
                  , type' "button"
                  , name "Next Month"
                  , onClick address (NextMonth date)
                  ]
                  []
              ]
           ,   [ input 
...
infoFooter =
    footer [ id "info" ]
      ...
      , p []
          [ text "Modified by "
          , a [ href "https://github.com/jester-euborn/" ] [ text "Melchor Vazquez" ]
          ]
      , p []
          [ text "Part of "
